...
/**
 * ConcurrentPool 异步 borrow 热路径基准。
 */
...
public class ConcurrentPoolBorrowAsyncJmh {
    ...
    public static class PoolState {
        ...
        public void setup() throws Exception {
            ...
            pool = new ConcurrentPool<>(config,
            ...
            for (int i = 0; i < maxPoolSize; i++) {
                entries.add(pool.borrowAsync(200, TimeUnit.MILLISECONDS)
    ...
    public static class SaturatedPoolState {
        ...
        public void setup() throws Exception {
            ...
            if ("executor".equals(timeoutScheduler)) {
                ...
                pool = new ConcurrentPool<>(config, factory,
            ...
            for (int i = 0; i < maxPoolSize; i++) {
                heldEntries.add(pool.borrowAsync(200, TimeUnit.MILLISECONDS)
                        .get(1, TimeUnit.SECONDS));
            }
        }
        ...
        public void restoreSaturatedState() throws Exception {
            // 触发一次 requite->borrowAsync，可惰性清理已超时/已取消等待节点，并恢复池满状态。
            TestPoolEntry entry = heldEntries.get(0);
            pool.requite(entry);
            TestPoolEntry reBorrowed = pool.borrowAsync(200, TimeUnit.MILLISECONDS)
                    .get(1, TimeUnit.SECONDS);
    ...
    static final class ExecutorTimeoutScheduler
        ...
        private final ScheduledExecutorService scheduler =
                Executors.newSingleThreadScheduledExecutor(r -> {
                    Thread th = new Thread(r, "pool-timeout-executor");
                    th.setDaemon(true);
                    return th;
                });
