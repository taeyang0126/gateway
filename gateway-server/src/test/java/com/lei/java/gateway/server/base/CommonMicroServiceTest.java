...
public class CommonMicroServiceTest {
    ...
    public static final GenericContainer<?> HTTPBIN_CONTAINER =
            new GenericContainer<>("kennethreitz/httpbin:latest").withExposedPorts(HTTPBIN_PORT)
                    .withNetwork(NETWORK)
                    .withNetworkAliases(HTTPBIN);
    /**
     * <a href="https://java.testcontainers.org/modules/toxiproxy/">toxiproxy</a> 使用 toxiproxy 封装
     * httpbin 可以使用 toxiproxy 模拟网络故障等情况 可以用的 port 范围是 8666～8697 也连接到同一个网络 一个 TOXIPROXY_CONTAINER
     * 对应多个不同 proxy，通过 TOXIPROXY_CONTAINER.getMappedPort(内部端口) 获取映射到主机的端口
     */
    ...
    static {
        ...
        try {
            ...
            Proxy proxy = toxiproxyClient.createProxy("good",
                    "0.0.0.0:"
                            + GOOD_HTTPBIN_PROXY_PORT,
                    HTTPBIN
                            + ":"
                            + HTTPBIN_PORT);
            ...
            proxy = toxiproxyClient.createProxy("read_timeout",
                    "0.0.0.0:"
                            + READ_TIMEOUT_HTTPBIN_PROXY_PORT,
                    HTTPBIN
                            + ":"
                            + HTTPBIN_PORT);
            // 将上下行带宽设为0，模拟读取超时
            ...
            proxy.toxics()
                    .bandwidth("UP_DISABLE", ToxicDirection.UPSTREAM, 0);
            proxy.toxics()
            ...
            proxy = toxiproxyClient.createProxy("reset_peer",
                    ...
                    HTTPBIN
                            ...
                            + HTTPBIN_PORT);
            // 在连接建立后立即重置连接
            // 上游重置 (ToxicDirection.UPSTREAM): 当客户端尝试向服务器发送数据时，连接会被重置，客户端会收到 "Connection reset by
            // peer" 错误
            // 下游重置 (ToxicDirection.DOWNSTREAM): 当服务器尝试向客户端发送数据时，连接会被重置，服务器会收到 "Connection reset by
            // peer" 错误
            // 延迟为1ms
            proxy.toxics()
                    .resetPeer("UP_SLOW_CLOSE", ToxicDirection.UPSTREAM, 1);
            proxy.toxics()
                    .resetPeer("DOWN_SLOW_CLOSE", ToxicDirection.DOWNSTREAM, 1);
        } catch (IOException e) {
